...
public class Main {
	...
	private static void initialize(int node, int begin, int end, Node[] tree,
		...
		if (begin == end) {
			...
			int nodeShift = node<<1;
			int indexShift = (begin+end)>>1;
			initialize(nodeShift, begin, indexShift, tree, prefixSum);
			initialize(nodeShift+1, indexShift + 1, end, tree,
					prefixSum);

			tree[node].max = max(tree[nodeShift].max, tree[nodeShift + 1].max);
			tree[node].min = min(tree[nodeShift].min, tree[nodeShift + 1].min);
			tree[node].maxSum = max(tree[nodeShift].maxSum,
									max(tree[nodeShift+ 1].maxSum,
											tree[nodeShift + 1].max
												- tree[nodeShift].min));
		}
	}
	...
	private static Node query(int node, int begin, int end, int i, int j,
		...
		int nodeShift = node<<1;
		int indexShift = (begin+end)>>1;
		Node n1 = query(nodeShift, begin, indexShift, i, j, tree);
		Node n2 = query(nodeShift + 1, indexShift + 1, end, i, j,
				tree);
