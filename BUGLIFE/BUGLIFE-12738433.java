...
public static void main(String[] args) {
	...
	for(int t=1;t<=tcs;++t)
	{
		...
		for(int i=1;i<=totalBugs;++i)
		{
			...
			{
				setNumbers[i] = 0;
				dfs.add(new int[]{i,setNumbers[i]});
				if(!checkCurrentList(visitedList, adjList, edgeCount, dfs, setNumbers))
				{
					noInteraction = false;
					break;
				}
...
private static boolean checkCurrentList(boolean[] visitedList, int[][] adjList, int[] edgeCount, Stack<int[]> dfs, int[] setNumbers)
{
	while(!dfs.isEmpty())
	{
		int[] currentBugInfo = dfs.pop();
		...
		if(!visitedList[currentBugInfo[ID]])
		{
			...
			for(int i=0,j=edgeCount[currentBugInfo[ID]];i<j;++i)
			{
				...
				dfs.add(new int[]{adjVertices[i],verticesSetNumber});
